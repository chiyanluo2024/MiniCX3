...
namespace client {
    namespace code_gen
    {
        ...
        bool compiler::operator()(ast::variable_declaration const& x)
        {
            ...
            if (p != 0)
            {
                error_handler(x.lhs, "Duplicate variable: " + x.lhs.name);
                return false;
            }
        ...
        bool compiler::operator()(ast::if_statement const& x)
        {
            ...
            if (!(*this)(x.then))
                return false;
            (*current)[skip] = int(current->size()) - int(skip);    // now we know where to jump to (after the if branch)
            ...
            if (x.else_)                                // We got an else
            {
                ...
                if (!(*this)(*x.else_))
                    return false;
                (*current)[exit] = int(current->size()) - int(exit);// now we know where to jump to (after the else branch)
            }
        ...
        bool compiler::operator()(ast::while_statement const& x)
        {
            ...
            (*current)[exit] = int(current->size()) - int(exit);    // now we know where to jump to (to exit the loop)
            return true;
        }
        ...
        bool compiler::operator()(ast::return_statement const& x)
        {
            if (void_return)
            {
                if (x.expr)
                {
                    error_handler(x, "'void' function returning a value: ");
                    return false;
                }
            }
            else
            {
                if (!x.expr)
                {
                    error_handler(x, current_function_name + " function must return a value: ");
                    return false;
                }
            }
        ...
        bool compiler::operator()(ast::function const& x)
        {
            ...
            if (functions.find(x.function_name.name) != functions.end())
            {
                error_handler(x.function_name, "Duplicate function: " + x.function_name.name);
                return false;
            }
            boost::shared_ptr<code_gen::function>& p = functions[x.function_name.name];
            p.reset(new code_gen::function(code, int(x.args.size())));
            current = p.get();
            ...
            if (!(*this)(x.body))
                return false;
            (*current)[1] = int(current->nvars());   // now store the actual number of variables
        ...
        bool compiler::operator()(ast::function_list const& x)
        {
            ...
            code[1] = int(p->get_address()) - 1; // jump to this (main function) address

            return true;
        }

        bool compiler::start(ast::statement_list const& x) {
            ast::function f = { "void", ast::identifier("main"), std::list<ast::identifier>(), x };
            ast::function_list functions({ f });
            return (*this)(functions);          
        }

        void compiler::print_variables(std::vector<int> const& stack) const {
            ...
            BOOST_FOREACH(pair const& p, functions)
            {
                ...
                p.second->print_variables(stack);
            }
        }
